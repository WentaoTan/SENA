...
class DataArguments:
    ...
    # SENA scoring
    scoring: Optional[bool] = field(default=False, metadata={"help": "whether to scores preferance data."})
...
class LazySupervisedDataset(Dataset):
    ...
    def data_preprocess(self, our_data, our_data_path):
        ...
        for idx in range(len(our_data)):
            ...
            chosen = our_data[idx]["chosen"].replace('</s>',"")
            reject = our_data[idx]["reject"].replace('</s>',"")
            question = our_data[idx]["question"].replace('</s>',"")
            question = "<image>\n" + question
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        data_dict['item'] = self.list_data_dict[i]
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'images' in instances[0]:
            ...
            if all(x is not None and x.shape == images[0].shape for x in images):
                ...
                batch['images'] = images
        batch['item'] = [instance['item'] for instance in instances]
        return batch


def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                data_args, scores=False) -> Dict:
...
def main():
    ...
    if data_args.scoring:
        with torch.no_grad():
            dpo_trainer.scoring()
            print('remember to python tool.py for convert scoring results to training file')
            return
